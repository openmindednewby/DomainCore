# Building and Deploying DomainCore
...
dotnet add package DomainCore --version 1.0.0 --source LocalFeed
```
...
```xml
<ItemGroup>
  <PackageReference Include="DomainCore" Version="1.0.0">
    <Source>C:\path\to\DomainCore\artifacts\DomainCore.1.0.0.nupkg</Source>
  </PackageReference>
</ItemGroup>
```
...
2. Click **Create**
3. Provide a key name (e.g., "DomainCore")
4. Select **Push** scope
5. Select packages or glob pattern (e.g., `DomainCore*`)
6. Copy the generated API key

### Step 2: Publish

```bash
dotnet nuget push ./artifacts/DomainCore.1.0.0.nupkg \
...
https://www.nuget.org/packages/DomainCore
...
unzip DomainCore.1.0.0.nupkg -d extracted/
...
```
error: Response status code does not indicate success: 409 (Conflict - The feed already contains 'DomainCore' version '1.0.0'.)
```
